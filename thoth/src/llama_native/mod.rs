//! Bindings to llama.cpp via bindgen
...
impl Default for Config {
    fn default() -> Self {
        Self {
            n_ctx: 2048, n_gpu_layers: 99, n_threads: 8, n_batch: 512,
...
impl Engine {
    pub fn load<P: AsRef<Path>>(path: P, config: Config) -> Result<Self> {
        ...
        if model.is_null() { anyhow::bail!("Failed to load model: {}", path.display()); }
        ...
        if ctx.is_null() { unsafe { llama_model_free(model) }; anyhow::bail!("Failed to create context"); }
        let sampler = unsafe { llama_sampler_chain_init(llama_sampler_chain_default_params()) };
        if sampler.is_null() { unsafe { llama_free(ctx); llama_model_free(model) }; anyhow::bail!("Failed to create sampler chain"); }
        let t = unsafe { llama_sampler_init_temp(config.temperature) };
        if !t.is_null() { unsafe { llama_sampler_chain_add(sampler, t); } }
        ...
        if !t.is_null() { unsafe { llama_sampler_chain_add(sampler, t); } }
        Ok(Self { model, ctx, sampler, config })
    }
    ...
    pub fn reset(&mut self) -> Result<()> {
        unsafe {
            ...
            llama_free(self.ctx);
            // Recreate context from model with same config
    ...
    where F: FnMut(StreamEvent) -> Result<(), ()> {
        ...
        let n_tokens = unsafe { llama_tokenize(vocab, cstr.as_ptr(), prompt.len() as i32, tokens.as_mut_ptr(), tokens.len() as i32, false, true) };
        if n_tokens < 0 { anyhow::bail!("Tokenization failed"); }
        tokens.truncate(n_tokens as usize);
        let batch = unsafe { llama_batch_get_one(tokens.as_mut_ptr(), n_tokens) };
        if unsafe { llama_decode(self.ctx, batch) } != 0 { anyhow::bail!("Prompt decode failed"); }
let eos = unsafe { llama_token_eos(vocab) };
    let mut output = String::new();
    let max_new = (self.config.n_ctx as i32 - n_tokens as i32).min(256).max(1);
    for _ in 0..max_new {
        let token = unsafe { llama_sampler_sample(self.sampler, self.ctx, -1) };
        if token == eos { break; }
        let mut buf = [0u8; 32];
        let n = unsafe { llama_detokenize(vocab, &token, 1, buf.as_mut_ptr() as *mut i8, buf.len() as i32, false, true) };
        if n > 0 {
...
impl Drop for Engine {
    fn drop(&mut self) {
        unsafe {
            if !self.sampler.is_null() { llama_sampler_free(self.sampler); }
            ...
            match rx.try_recv() {
                Ok(Command::Load(path, config, resp)) => {
                    ...
                    match Engine::load(&path, config) {
                        Ok(e) => { unsafe { *engine_ptr = Some(e); } let _ = resp.send(Ok(())); }
                ...
                Ok(Command::InferStream(prompt, event_tx, resp)) => {
                    if let Some(ref mut e) = unsafe { (*engine_ptr).as_mut() } {
                        ...
                        let cb = |event: StreamEvent| {
                            if tx.send(event).is_err() { return Err(()); }
                            Ok(())
                        };
                        match e.infer_stream(&prompt, cb) {
                            Ok(()) => { let _ = resp.send(Ok(())); }
                            Err(err) => {
                                let _ = event_tx.send(StreamEvent::Error(format!("Inference error: {}", err)));
...
pub fn infer_stream(
    handle: &std::sync::Arc<std::sync::Mutex<Option<std::sync::mpsc::Sender<Command>>>>,
    prompt: String,
) -> Result<(tokio::sync::mpsc::UnboundedReceiver<StreamEvent>, tokio::sync::oneshot::Receiver<Result<()>>)> {
    let guard = handle.lock().unwrap();
    let tx = guard.as_ref().ok_or_else(|| anyhow::anyhow!("Engine not started"))?;
