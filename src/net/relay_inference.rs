...
pub struct InferenceRelay {
    ...
    mls: Arc<Mutex<crate::net::MlsGroupManager>>,
}
...
impl InferenceRelay {
    pub fn new() -> Self {
        Self {
            ...
            mls: Arc::new(Mutex::new(crate::net::MlsGroupManager::new())),
        }
    }
    ...
    async fn send_group_message(&self, target_pubkey: &str, msg: &GroupMessage) -> Result<()> {
        ...
        let ciphertext = {
            let mut mls = self.mls.lock().await;
            mls.encrypt(DEVICE_GROUP_ID, &plaintext)?
        };
    ...
    async fn decrypt_group_message(&self, ciphertext: &[u8]) -> Result<GroupMessage> {
        let mut mls = self.mls.lock().await;
        let plaintext = mls.decrypt(DEVICE_GROUP_ID, ciphertext)?;
    ...
    pub async fn send_inference_request(&self, target_pubkey: &str, req: InferenceRequest) -> Result<tokio::sync::oneshot::Receiver<InferenceResponse>> {
        ...
        self.send_group_message(target_pubkey, &GroupMessage::InferenceRequest(req.clone())).await?;
    ...
    pub async fn send_inference_response(&self, target_pubkey: &str, resp: InferenceResponse) -> Result<()> {
        self.send_group_message(target_pubkey, &GroupMessage::InferenceResponse(resp.clone())).await?;
        info!("Sent inference response {} to {} (MLS-encrypted)", resp.request_id, target_pubkey);
        Ok(())
    }

    pub async fn send_text(&self, target_pubkey: &str, content: String) -> Result<()> {
        self.send_group_message(target_pubkey, &GroupMessage::TextMessage { content }).await
    }
    ...
    pub async fn handle_event(&self, event: &nostr_sdk::Event) -> Result<RelayEvent> {
        match event.kind {
            ...
            k if k == KIND_MLS_INVITE => {
                ...
                let mut mls = self.mls.lock().await;
                let joined_group = mls.process_welcome(&welcome_bytes, event.pubkey.to_hex())?;
                Ok(RelayEvent::MlsInvite {
                    sender: event.pubkey.to_hex(),
                    group_id: joined_group,
                    welcome_bytes,
                })
            }
            Kind::GiftWrap => {
                ...
                match self.decrypt_group_message(&ciphertext).await {
                    Ok(group_msg) => {
                        ...
                        match group_msg {
                            ...
                            GroupMessage::TextMessage { content } => {
                                info!("Received group text from {} (MLS-decrypted)", sender);
                                Ok(RelayEvent::GroupText { sender, content })
                            }
                    ...
                    Err(e) => {
                        warn!("Failed to decrypt group message from {}: {} — may be a non-group GiftWrap", event.pubkey, e);
                        Err(anyhow!("Group message decryption failed: {}", e))
                    }
                }
            }
    ...
    pub async fn add_device_to_group(&self, member_pubkey: &str) -> Result<(Vec<u8>, Vec<u8>)> {
        let mut mls = self.mls.lock().await;
        let key_package = mls.generate_key_package(member_pubkey.to_string())?;
        let (commit, welcome) = mls.add_member(DEVICE_GROUP_ID, &key_package, member_pubkey.to_string())?;
        info!("Added device {} to group '{}'", member_pubkey, DEVICE_GROUP_ID);
        Ok((commit, welcome))
    }
}
...
mod tests {
    ...
    fn test_device_caps_score() {
        let desktop = DeviceCaps {
            device_name: "desktop".into(), pubkey: "pk1".into(),
            gpu_layers: 99, ram_mb: 32768, cpu_cores: 16,
            model_loaded: Some("Bonsai-1.7B".into()),
            is_desktop: true, supports_inference: true, timestamp: 0,
        };
        let phone = DeviceCaps {
            device_name: "phone".into(), pubkey: "pk2".into(),
            gpu_layers: 0, ram_mb: 8192, cpu_cores: 4,
            model_loaded: Some("Bonsai-1.7B".into()),
            is_desktop: false, supports_inference: true, timestamp: 0,
        };
    ...
    fn test_group_message_serde() {
        let req = InferenceRequest {
            ...
            prompt_segments: vec!["hello".into()],
            system_prompt: "You are Tot.".into(),
            model_hint: Some("Bonsai".into()),
            max_tokens: Some(512),
    ...
    fn test_inference_response_serde() {
        let resp = InferenceResponse {
            request_id: "test-456".into(), content: "Hello!".into(),
            thinking: "Hmm...".into(), model_used: "Bonsai-1.7B".into(),
            tokens_generated: 10, elapsed_ms: 500,
        };
